...
- Управляемый каталог `Applications/` является единственным источником
  приложений; Windows Shell API извлекает иконки и запускает выбранный элемент.
...
- Каталог `Applications/` сканируется до показа окна и по `F5`.
...
- Удаление из UI ограничено каноническими обычными файлами внутри
  ...
  после чего каталог и раскладка согласуются заново.
...
`%LOCALAPPDATA%\Programs\Windows Launchpad`, переносит существующие
`Applications/` и `LaunchpadLayout.store` только при первой установке и
создаёт Shell-ярлыки через `IShellLinkW + IPropertyStore`. Благодаря одинаковому
